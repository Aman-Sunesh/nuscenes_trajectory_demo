...
class TransformerBaseline(nn.Module):
    ...
    def forward(self, x):
        ...
        # Use the last observed timestep representation
...
def plot_metric_comparison(results):
    ...
    plt.ylabel("Error")
    plt.title("Transformer Baseline Comparison")
    plt.legend()
    plt.grid(axis="y")
    plt.tight_layout()
...
def run_transformer(
    ...
    method_name = f"transformer_d{d_model}_heads_{nhead}_layers_{num_layers}_ff_{dim_feedforward}_lr_{lr_name}"
...
def main():
    ...
    results.append(run_transformer(d_model=64, nhead=4, num_layers=1, dim_feedforward=128, epochs=100, lr=3e-4))
    results.append(run_transformer(d_model=128, nhead=4, num_layers=1, dim_feedforward=256, epochs=100, lr=3e-4))
    results.append(run_transformer(d_model=128, nhead=4, num_layers=2, dim_feedforward=256, epochs=100, lr=3e-4))
    results.append(run_transformer(d_model=128, nhead=4, num_layers=2, dim_feedforward=512, epochs=100, lr=3e-4))
